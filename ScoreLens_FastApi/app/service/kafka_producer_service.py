from ScoreLens_FastApi.app.config.kafka_consumer_config import TOPIC_CONSUMER
...
def send_heart_beat(msg: ProducerRequest):
    try:
        p = producer()
        p.send("ping", value=msg.model_dump_json(), partition=0)
        p.flush()
        logger.info(f"Sent Kafka message to topic ping: {msg}")
    except Exception as e:
        logger.exception(f"Failed to send message to Kafka: {e}")
        raise AppException(
            status_code=500,
